...
def generateMAL(out_excel, county, rate=10.0, year=2015, where_clause='', sort_by=DEFAULT_SORT):
    ...
    sql = (None, 'ORDER BY {}'.format(', '.join(sort_by.split(';') if isinstance(sort_by, basestring) else sort_by)))
    ...
    summary_clause = "{} = '{}' AND ".format(COUNTY_GIS, county.upper()) + no_flag + null_secs
    ...
    with arcpy.da.SearchCursor(gdb.summary_table, summary_fields, summary_clause) as rows:
        for r in rows:
    ...
    with arcpy.da.SearchCursor(gdb.breakdown_table, fields + [PIN], where_clause=where_clause + null_secs, sql_clause=sql) as rows:
        for r in rows:
